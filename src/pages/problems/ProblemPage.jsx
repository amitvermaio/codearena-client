...
// --- UI styles pulled into a constant (small tidy)
...
// small no-op telemetry initializer (harmless)
const initPageTelemetry = () => {}
initPageTelemetry()

/**
 * TagMultiSelect
 * Extracted helper component — tiny, sensible refactor that is common in maintenance.
 */
const TagMultiSelect = ({ tagsArr = [], value = [], onChange }) => {
  const options = tagsArr.map((tag) => ({ value: tag, label: tag }))
  const selected = value.map((t) => ({ value: t, label: t }))
  return (
    <Select
      isMulti
      placeholder='Tags'
    ...
    <>
      ...
      <div className='container mx-auto py-8 px-4 md:px-6'>
        ...
        <div className='mb-6 flex flex-col md:flex-row items-center gap-4'>
          ...
          {/* DIFFICULTY */}
          <ShadcnSelect value={filters.difficulty} onValueChange={(v) => handleFilter('difficulty', v)}>
          ...
          {/* STATUS */}
          <ShadcnSelect value={filters.status} onValueChange={(v) => handleFilter('status', v)}>
            <SelectTrigger className='w-full sm:w-[150px]'>
          ...
          <TagMultiSelect
            tagsArr={tagsArr}
            value={filters.tags}
            onChange={(vals) => handleFilter('tags', vals)}
          />
        </div>
